...
const Cart = ({ cartNum, setCartNum }) => {
    ...
    const [canAffordCart, setCanAffordCart] = useState(false)
    const [, setTotalPrice] = useState(0);

    useEffect(() => {
        const checkWalletBalance = async () => {
            const walletBalance = await connection.getBalance(publicKey);
            const solTotalPrice = priceList.reduce((sum, token) => sum + token.price, 0);

            // Since getBalance returns in Lamports but priceList contains Sol prices
            setCanAffordCart(walletBalance / LAMPORTS_PER_SOL >= solTotalPrice);
        };

        checkWalletBalance();
    }, [connection, publicKey, priceList]);
                        ...
                        : <div className='flex flex-col md:flex-row'>
                            ...
                            </div>
                            {
                                ...
                                <div className='w-full md:w-3/12 md:ml-8 px-4 flex flex-col mb-8'>
                                    ...
                                    <Tooltip
                                        title="You do not have sufficient balance in connected wallet to perform this transaction."
                                        disableHoverListener={canAffordCart}
                                    >
                                        <span> {/* Wrapping Button in a <span> to apply tooltip to disabled Button */}
                                            <Button
                                    ...
                                    {/* <Button onClick={handleBuyAll} disabled={!canAffordCart} className={classes.button} variant="outlined">Buy All</Button> */}
                                </div>
                            }
                        </div>
